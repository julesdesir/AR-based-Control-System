...
targetUpdateTime = 20; % in seconds
fps = 60; % Frames per second, i.e, 60 data points per s

drillTime = 2; % total simulation time, min
timeplot = linspace(0,7200,7200)./3600; %min 
...
motorRPM = NaN(1,7200);
WOBControl = NaN(1,7200);
chokeControl = NaN(1,7200);
pumpControl = NaN(1,7200);

user_data = NaN(7200,13);
...
while true
    ...
    % end
    % Mechanics
    [t, x_np1] = ode15s(@(t,x)drillstring(t,x,RPM,WOB,P), t_compute, x_n);
    x_n = x_np1(end,:);
    bitRPM = x_np1(end,43)*rad2rpm;

    % Output to gauge displays
    ...
    % y = [bitRPM, qres, BHCP, pp, pc, targetRPM, targetWOB, targetPump, targetChoke]
    ...
    % Reply to Client
    write(server,y,'single',data.SenderAddress,data.SenderPort);
    % disp('Server replied')
    ...
    if rem(i,fps) == 0
        ...
        if i<=di
            ...
            subplot(2,3,[4,6])
            yyaxis left
            plot(timeplot(1:i),pumpControl(1:i), 'LineWidth',2)
            ylabel('Pump Flowrate, lit/min')

            yyaxis right
            plot(timeplot(1:i),chokeControl(1:i),'LineWidth',2)
            ylabel('Choke Opening, [-]')
            ...
            subplot(2,3,[4,6])
            yyaxis left
            plot(timeplot(i-di:i),pumpControl(i-di:i),'LineWidth',2)
            ylabel('Pump Flowrate, lit/min')

            yyaxis right
            plot(timeplot(i-di:i),chokeControl(i-di:i),'LineWidth',2)
            ylabel('Choke Opening, [-]')
...
% end
%%
% save("user_1_scenario_1.txt","user_data","-ascii")
...
%         % figure(3)
%         % plot(bitVelocity(1:i),depth(1:i),'linewidth',2);
%         % addaxis(bitVelocity(1:i)+10,depth(1:i),'linewidth',2);
%         % 
%         % addaxislabel(1,'Bit Velocity')
%         % addaxislabel(2,'Bit Velocity +10')
%         % 
%         % axis([0 400 depth(1) depth(di)])
%         % set(gca,'XAxisLocation','top','YAxisLocation','left','ydir','reverse');
%         % drawnow
% 
%         % figure(3)
%         % plot(reservoirFlow(1:i),depth(1:i))
%         % % axis([-2 2 depth(1) depth(di)])
%         % ylim([depth(1) depth(di)])
%         % set(gca,'XAxisLocation','top','YAxisLocation','left','ydir','reverse');
%         % drawnow
...
% 
%         % figure(2)
%         % plot(reservoirFlow(i-di:i),depth(i-di:i))
%         % % axis([-2 2 depth(i-di) depth(i)])
%         % ylim([depth(i-di) depth(i)])
%         % set(gca,'XAxisLocation','top','YAxisLocation','left','ydir','reverse');
%         % drawnow
